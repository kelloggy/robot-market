...
const RobotMarket = () => {
  ...
  const [temp, setTemp] = React.useState([])
    ...
    React.useEffect(() => {
      let url = `http://localhost:8000/api/robots/?material=Rubber`;
      let config = {
          method: 'GET',
          timeout: 1000 * 3,
          headers: {
          ...
          if (response.status >= 200 && response.status <= 299) {
              ...
              if (filterArr.length== 0) {
                ...
                console.log('filter arr NOT empty')
                 ...
                 const resulted = response.data.data.map((val) => {
                  const result = filterArr.filter((fa) => fa == val.material)
                  temp.push(result)
                  return temp
                })
  ...
  return (
    ...
    <div className={classes.root}>
      ...
      <AppBar position="fixed">
        {console.log("filter robots", robots)}
        <Toolbar>
